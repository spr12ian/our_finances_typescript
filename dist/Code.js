...
class BankDebitsDue {
    ...
    getUpcomingDebits() {
        ...
        scheduledTransactions.forEach(transaction => {
            var _a;
            const accountKey = (_a = transaction[BankDebitsDue.COL_ACCOUNT_KEY]) === null || _a === void 0 ? void 0 : _a.trim(); // Optional chaining and trim
...
class BudgetAnnualTransactions {
    ...
    getUpcomingDebits() {
        ...
        scheduledTransactions.forEach(transaction => {
            const { [BudgetAnnualTransactions.COLUMNS.DATE]: date, [BudgetAnnualTransactions.COLUMNS.CHANGE_AMOUNT]: changeAmount, [BudgetAnnualTransactions.COLUMNS.DESCRIPTION]: description, [BudgetAnnualTransactions.COLUMNS.FROM_ACCOUNT]: fromAccount, [BudgetAnnualTransactions.COLUMNS.PAYMENT_TYPE]: paymentType } = transaction;
...
class CheckFixedAmounts {
    ...
    createMismatchMessage(row) {
        ...
        return Utilities.formatString('%s %s: Dynamic amount (%s) does not match fixed amount (%s)', row[columns.TAX_YEAR], row[columns.CATEGORY], getAmountAsGBP(row[columns.DYNAMIC_AMOUNT]), getAmountAsGBP(row[columns.FIXED_AMOUNT]));
    }
    getEmailBody() {
...
class HMRC_S {
    ...
    handleEdit(trigger) {
        try {
            ...
            if (!value || row <= HMRC_S.SHEET.HEADER_ROW)
                return;
...
class Spreadsheet {
    ...
    getSheetMap() {
        if (!this._sheetMap) {
            ...
            this._sheetMap = Object.fromEntries(sheets.map(sheet => [sheet.getName(), sheet]));
        }
...
function getMyEmailAddress() {
    ...
    const myEmailAddress = (_a = getPrivateData()) === null || _a === void 0 ? void 0 : _a['MY_EMAIL_ADDRESS'];
